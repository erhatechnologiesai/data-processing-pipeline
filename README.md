# Tabular Data Cleansing Pipeline
...
An intelligent data cleansing pipeline that inspects messy tabular datasets, detects statistical anomalies, handles missing values, and outputs audit summaries.

---

## Key Features

- **Automated**: tabular schema inference (dates, currencies, numerics, text)
- **Missing**: value imputation strategies (mean, median, mode, forward-fill)
- **Statistical**: outlier and anomaly detection using IQR and z-score methods
- **Data**: quality scorecard showing cleanliness percentage and remediation steps
- **Clean**: CSV and JSON export endpoints
...
```mermaid
flowchart TD
    RawData([Raw Messy Records]) --> Schema[Schema Inference]
    Schema --> AnomalyScan[Anomaly & Outlier Scanner]
    AnomalyScan --> Imputer[Missing Value Imputer]
    Imputer --> QualityReport[Data Quality Audit Report]
    QualityReport --> CleanData[Clean Dataset Output]
```
...
1. **Clone the repository:**
   ```bash
   git clone https://github.com/erhatechnologiesai/data-processing-pipeline.git
...
|---|---|---|
| `POST` | `/clean-data` | Clean tabular data, impute missing values, and report anomalies |

### Example Request

```bash
curl -X POST http://127.0.0.1:8000/clean-data -H "Content-Type: application/json" -d '{"records": [{"id": 1, "val": 100}, {"id": 2, "val": null}, {"id": 3, "val": 99999}]}'
```
